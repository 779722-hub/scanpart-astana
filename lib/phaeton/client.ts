...
// Serverless-компромисс: тёплое соединение (его держит /api/cron/warm) отдаёт
// даже большой ответ цен (~230КБ) за ~2-3с — тогда Phaeton в выдаче. На совсем
// «холодном» соединении (первый запрос к новому инстансу после деплоя/простоя)
// эти 230КБ по прокси КЗ идут >20с — их не спасает ни бОльший таймаут, ни
// повтор (проверено: 20с+повтор = 42с И всё равно без Р1). Поэтому таймаут 10с
// и НЕ повторяем на таймаут (только на быстрые сетевые сбои ниже): холодный
// вызов не блокирует поиск, а Р1 подхватится, как только соединение прогрето.
// Настоящая стабильность Phaeton = постоянный процесс (VPS), а не serverless.
...
function isTransient(err: Error): boolean {
  const m = err.message;
  // Таймаут/abort НЕ считаем транзиентом: повтор по холодному соединению всё
  // равно не успевает и лишь удваивает ожидание.
  return (
