...
// 20s: большие ответы цен по ходовым артикулам (Phaeton отдаёт 300+ позиций,
// ~220КБ) на «холодном» соединении с прокси КЗ доходят за ~16-17с; при 8с их
// отбрасывало (ретрая на таймаут нет) → Phaeton/Р1 пропадал через раз. 20с
// покрывает даже холодное соединение. Тёплые вызовы (прогрев /api/cron/warm)
// ~2-3с — запас им не мешает. Ретрая на таймаут нет, так что худший случай
// ограничен 20с (а не 40с, как было с 20с + ретрай).
