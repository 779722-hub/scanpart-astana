...
// даже большой ответ цен (~230КБ) за ~2-3с. На совсем «холодном» соединении
// (первый запрос к новому инстансу после деплоя/простоя) эти 230КБ по прокси КЗ
// идут >20с. Теперь Phaeton (Р1) грузится ТОЛЬКО в фоновой фазе поиска
// (?phase=phaeton) и НЕ блокирует быструю выдачу, поэтому таймаут щедрый (25с)
// и мы ПОВТОРЯЕМ на таймаут/abort: фон может подождать, зато Р1 доезжает даже
// на холодном инстансе. Настоящая стабильность = постоянный процесс (VPS).
...
function isTransient(err: Error): boolean {
  const m = err.message;
  // Фоновая фаза не торопится — повтор по таймауту/abort безопасен: второе
  // соединение обычно уже прогрето и успевает докачать ~230КБ.
  return (
    /ECONN|ENOTFOUND|EAI_AGAIN|fetch failed|socket hang up/i.test(m) ||
    /abort|timed?\s*out/i.test(m) ||
